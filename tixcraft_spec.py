...
class TicketBot:
        ...
        try:
            if await self.wait_for_clickable("select"):
                ...
                if tixcraft_setting.TICKET_NUMBER in options:
                    await self.page.locator('select').first.select_option(tixcraft_setting.TICKET_NUMBER)
                else:
    ...
    async def choose_ticket_count(self):
        ...
        try:
            if await self.wait_for_clickable("select"):
                await self.page.locator('select').first.select_option(tixcraft_setting.TICKET_NUMBER,timeout=600)
